 ...
 * Time: 17:52
 */
namespace liupanv\think\theme\library;
use think\App;
use think\Config;
...
class Templete
{
     ...
     * @return string|false
     */
    private function parseTemplateFile($template, $config = [], $jumpDefaultTemplate = 1)
    {
        ...
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            if (0 !== strpos($template, '/')) {
                $template = str_replace(['/', ':'], $config['view_depr'], $template);
            } else {
                $template = str_replace(['/', ':'], $config['view_depr'], substr($template, 1));
            }
            ...
            if ($config['view_base']) {
                ...
                $path = isset($module) ? $this->app->getAppPath() . $module . DIRECTORY_SEPARATOR . basename($config['view_path']) . DIRECTORY_SEPARATOR : $config['view_path'];
            }

            $template = $path . $template . '.' . ltrim($config['view_suffix'], '.');
        }
        ...
        if($jumpDefaultTemplate)
        {
            ...
            return $this->parseTemplateFile($oriTemplate, $config, 0);
        }
