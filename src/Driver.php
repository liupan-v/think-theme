...
class Driver
{
    ...
    public function __construct($config = [])
    {
        ...
        if(!config('?think_theme.is_open') || !config('think_theme.is_open'))
        {
            if (empty($this->config['view_path'])) {
                $this->config['view_path'] = $modulePath . 'view' . DS;
            }
            $this->template = new TemplateDefault($this->config);
            return;
        }
        $this->config['view_path'] = $modulePath.config('think_theme.theme_path').DS.config('think_theme.current').DS;
     ...
     * @return string
     */
    private function parseTemplate($template, $isJumpDefault = 1)
    {
        ...
        $fullPath = $path . ltrim($template, '/') . '.' . ltrim($this->config['view_suffix'], '.');
        if (config('?think_theme.is_open') && config('think_theme.is_open') && $isJumpDefault && !is_file($fullPath)) {
            $this->config['view_path'] = App::$modulePath.config('think_theme.theme_path').DS.config('think_theme.default').DS;
            return $this->parseTemplate($oriTemplate,0);
        }
        return $fullPath;
    }
